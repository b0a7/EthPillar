...
class DirectoryListingError(OSError):
    """Raised when a directory exists but cannot be listed (including sudo find)."""
...
def list_dir_basenames(path: str) -> List[str]:
    ...
    Raises:
        DirectoryListingError: The path could not be listed (do not treat as empty).
    """
    try:
        ...
        if result.returncode != 0:
            ...
            raise DirectoryListingError(f"Unable to list directory {path}: {err}") from None
...
_VALIDATOR_API_RE = re.compile(r"--validator-api-address=(?P<addr>[^\s\\]+)")
_FEATURE_SET_RE = re.compile(r"--feature-set-enable=(?P<val>[^\s\\]+)")
JSON_REQUESTS_FEATURE = "json_requests"
...
def plan_cdvn_env_import(
    ...
    *,
    ...
    p2p_port_key = "CHARON_PORT_P2P_TCP" if (env.get("CHARON_PORT_P2P_TCP") or "").strip() else "CHARON_P2P_PORT"
    p2p_port = (env.get("CHARON_PORT_P2P_TCP") or env.get("CHARON_P2P_PORT") or "").strip()
    ...
    if p2p_tcp:
        ...
        plan.add_row(
            f"{p2p_port_key}={p2p_port}",
            f"--p2p-tcp-address={plan.p2p_tcp_address}",
        )
    else:
        plan.add_row(
            "# CHARON_PORT_P2P_TCP/CHARON_P2P_PORT unset",
            f"--p2p-tcp-address={plan.p2p_tcp_address}",
            kind="default",
        )
...
def scrape_validator_api_address(content: str) -> Optional[str]:
    """Extract ``--validator-api-address`` from a Charon unit file."""
    match = _VALIDATOR_API_RE.search(content)
    return match.group("addr") if match else None


def validator_api_url(address: str) -> str:
    """Turn a Charon validator-API bind (``host:port``) into an HTTP URL."""
    address = (address or "").strip()
    if not address:
        return DEFAULT_VALIDATOR_API_URL
    if address.startswith("http://") or address.startswith("https://"):
        return address
    return f"http://{address}"


def align_vc_beacon_to_charon_api(
    validator_api_address: str,
    vc_name: str,
    service_path: str = "/etc/systemd/system/validator.service",
) -> bool:
    """Point validator.service at Charon's validator-API after a CDVN env import.

    Args:
        validator_api_address: Charon ``--validator-api-address`` value.
        vc_name: EthPillar validator client name.
        service_path: Path to ``validator.service``.

    Returns:
        True when the VC unit was patched.
    """
    if not vc_name or not os.path.isfile(service_path):
        return False
    from deploy.vc_service import patch_beacon_endpoint

    return patch_beacon_endpoint(
        service_path, vc_name, validator_api_url(validator_api_address)
    )


def patch_json_requests_feature(service_path: str, *, enable: bool) -> bool:
    """Set or clear ``--feature-set-enable=json_requests`` on a Charon unit.

    After a consensus-client switch, Nimbus BNs need JSON beacon APIs; other
    BNs must drop a leftover ``json_requests`` flag.
    ...
    match = _FEATURE_SET_RE.search(content)
    current = [part for part in (match.group("val").split(",") if match else []) if part]
    has_json = JSON_REQUESTS_FEATURE in current

    if enable:
        if has_json:
            return False
        current.append(JSON_REQUESTS_FEATURE)
        new_flag = f"--feature-set-enable={','.join(current)}"
        if match:
            new_content = _FEATURE_SET_RE.sub(new_flag, content, count=1)
        elif _P2P_TCP_ADDRESS_RE.search(content):
            new_content = _P2P_TCP_ADDRESS_RE.sub(
        ...
        if not has_json:
            return False
        remaining = [part for part in current if part != JSON_REQUESTS_FEATURE]
        if remaining:
            new_content = _FEATURE_SET_RE.sub(
                f"--feature-set-enable={','.join(remaining)}",
                content,
                count=1,
            )
        else:
            new_content = re.sub(
                r"(?:\s*\\\s*\n\s*)?--feature-set-enable=" + re.escape(match.group("val")),
                "",
                content,
                count=1,
            )
