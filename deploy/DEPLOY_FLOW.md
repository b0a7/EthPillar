...
Runtime: `getValidatorMode()` stays `none | separate | integrated_grandine`. Detect Charon via `isCharonEnabled()` (`charon.service`). After a CC switch, `patchValidatorBeaconEndpoint` updates Charon’s `--beacon-node-endpoints` (VC stays on `:3600`), then Charon is `try-restart`ed. TUI **Migrate from CDVN** (`migrateCharonFromCdvn`) copies `.charon`, maps `.env` → systemd (`deploy.charon import_env`), and offers key-share import / Charon start.
