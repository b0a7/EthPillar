...
Runtime: `getValidatorMode()` stays `none | separate | integrated_grandine`. Detect Charon via `isCharonEnabled()` (`charon.service`). After a CC switch, `patchValidatorBeaconEndpoint` updates Charon’s `--beacon-node-endpoints` (VC stays on `:3600`), then Charon is `try-restart`ed. TUI **Import CDVN .env → systemd** (`deploy.charon import_env`) maps CDVN `CHARON_*` / `BUILDER_API_ENABLED` into `charon.service` flags.
