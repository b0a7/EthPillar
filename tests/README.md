...
- `tests/test_history_expiry_suggestions.bats`: History-expiry helper detection, ExecStart prune-flag merge, scan-friendly output, Nethermind 2.0 rolling flags, and Execution Client → Suggest pruning parameters menu placement.
...
- `tests/test_ethpillar_cli.bats`: Non-interactive CLI (help, status, start/stop/restart, logs → rolling consolidated `view_journal_logs`).
