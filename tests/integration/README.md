...
- `check_client_versions.sh`: After deploy (and after update tests), verifies installed versions parse via `getExecutionCurrentVersion` / `getClVcCurrentVersion` and match `release_info … LATEST` (same comparison as the update menu’s “already on latest” path). Upgrade-case deploy (`--test-updates`) may force an RC: post-deploy logs say `forced RC` for those clients; after upgrade the override is cleared and the check is official LATEST again.
- `latest_override.py` / `find_client_rc.py`: Test-only LATEST→RC remap used only during Upgrade-case **deploy**. `find_client_rc` only accepts prereleases that are **semver-newer than official LATEST** (same-series RCs like `1.48.0-rc.0` vs `1.48.0` are skipped). Nethermind is limited to client tags (`1.35.0-rc`), not `bootnode` / other org artifacts. Look for `Upgrade-case RC deploy override` in Integration logs (RC vs “no resolvable RC — deploy LATEST”).
- `sitecustomize.py`: Caches release **binaries** only (revalidated with `HEAD` before reuse). GitHub API / release metadata always hits the network. Also installs the Upgrade-case LATEST→RC hook (no-op unless the override file exists).
