...
- `latest_override.py` / `find_client_rc.py`: Test-only LATEST→RC remap used only during Upgrade-case **deploy**. Look for `Upgrade-case RC deploy override` in Integration logs (RC vs “no resolvable RC — deploy LATEST”).
