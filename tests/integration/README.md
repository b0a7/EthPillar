...
- `run_inside_docker.py`: Executes inside each container to run the deployment and verify artifacts via `systemctl`. Does not install Python deps itself. CL QUIC listen (`UDP 9001`) is required only when the installed consensus binary advertises EthPillar’s pinned flag in `--help` (Nimbus v26.7.0 does not; QUIC gossip is v26.8.0+). Awkward help falls back to the version floors documented in `ethpillar.sh`.
- `check_client_versions.sh`: After deploy (and after update tests), verifies installed versions parse via `getExecutionCurrentVersion` / `getClVcCurrentVersion` and match `release_info … LATEST` (same comparison as the update menu’s “already on latest” path). Upgrade-case deploy (`--test-updates`) may force a seed: post-deploy logs say `forced seed` for those clients (base semver ± commit; missing `-rc.N` on the binary is accepted); after upgrade the override is cleared and the check is official LATEST again.
- `latest_override.py` / `find_client_rc.py`: Test-only LATEST remap used only during Upgrade-case **deploy**. `find_upgrade_seed` prefers an RC that is **semver-newer than official LATEST**, otherwise the newest sane previous stable (same major, at most two minors behind; client-shaped / junk / Nethermind filters). Same-series RCs like `1.48.0-rc.0` vs `1.48.0` are skipped. Nethermind is limited to client tags (`1.35.0-rc`), not `bootnode` / other org artifacts. Look for `Upgrade-case seed deploy override` in Integration logs. No sane seed → deploy official LATEST and soft-skip the real-upgrade assert.
- `test_updates.sh`: Detects installed units via `/etc/systemd/system/<name>.service` (same as the CLI; `systemctl list-unit-files` needs dbus and fails as non-root). Seeded clients: first `ethpillar upgrade` must restart and land on LATEST; second must skip (`already up to date` + `check-updates` exit 0, PID unchanged).
- `sitecustomize.py`: Caches release **binaries** only (revalidated with `HEAD` before reuse). GitHub API / release metadata always hits the network. Also installs the Upgrade-case LATEST→seed hook (GitHub clients + Geth; no-op unless the override file exists).
